...
Your program and data reside together in atomspace as MeTTa can be both a programming language and a data construct, a knowledge graph. 
...
In MeTTa, the "code" is essentially rewrite rules and is stored in atomspace. Rewrite rules start with "(=". Given some input pattern, the right side of the rewrite rule is executed. The input pattern below is "(What is MeTTa?)."
...
When you click the "Run" button below MeTTa will add the "code" and "data" statements to atomspace. Then MeTTa executes the "!" statement which results in "(What is MeTTa?)" being matched with "(What is MeTTa?)" in the "(=" rewrite rule. The right side of the "(=" rewrite matches "(MeTTa is fun!)" returning the result, "fun."
...
A red parenthesis is often a tip-off that you have unbalanced parens somewhere in your expression.
