...
Functions, also referred to as rewrite rules and equalities, also use variables, and have the distinct form of:

\textbf{(= (pattern) (result))}

The following example matches two variations of a light switch pattern. If called with the pattern of switch on, it returns the result off, and vice versa. Note that when the function is submitted to MeTTa, it is added to atomspace, so if resubmitted, another version of the function is added. 
