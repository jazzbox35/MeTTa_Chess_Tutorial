...
With \textbf{if} statements MeTTa uses the functional style for boolean tests. Instead of coding "if \$a > 5..." as you would in a language like Python, place the test operator first, eg "(if (> \$a 5)...)" in MeTTa.  Multiple conditions result in expressions looking like the following example. The true expression immediately follows the conditional test and the false expression is second.
